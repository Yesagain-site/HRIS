...
// This function returns the display status for the table (always "Present" if employee showed up)
const getDisplayStatus = (status: AttendanceRecord['status']): string => {
    if (status === 'Present' || status === 'Late' || status === 'Early Departure') {
        return 'Present';
    }
    return status;
};

// This function determines if a record should be counted as "Present" in stats
const isPresentForStats = (status: AttendanceRecord['status']): boolean => {
    return status === 'Present' || status === 'Late' || status === 'Early Departure';
};
...
const ManagerDashboardTab: React.FC = () => {
    ...
    const todaySummary = useMemo(() => {
        ...
        return {
            ...
            // Present count includes everyone who showed up (Present, Late, Early Departure)
            present: todayRecords.filter((r) => 
                r.status === 'Present' || r.status === 'Late' || r.status === 'Early Departure'
            ).length,
        ...
        <div className="space-y-6">
            ...
            <Card title="Today's Attendance Snapshot">
                ...
                <div className="mt-2 text-xs text-gray-500 text-center">
                    * Present count includes employees who arrived late or left early
                </div>
            </Card>
                                        ...
                                        myRecords.map((rec) => (
                                            <tr key={`att-${rec.id}`} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-3">
                                                    {/* Always show "Present" for any status where employee showed up */}
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex flex-col gap-1">
                                                        ...
                                                        {rec.lateMinutes === 0 && rec.earlyDepartureMinutes === 0 && '—'}
                                                    </div>
                                                </td>
                                            </tr>
                                        ))
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
                                    <div>
                                        <div className="font-bold text-lg text-green-600">
                                            {/* Count ALL employees who showed up as Present (matches table display) */}
                                            {myRecords.filter((r) => isPresentForStats(r.status)).length}
                                        </div>
...
const EmployeeAttendanceHistory: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const savedDesignation = localStorage.getItem('attendance_history_designation');

        if (savedEmployeeId) setSelectedEmployeeId(savedEmployeeId);
        if (savedMonth) setSelectedMonth(parseInt(savedMonth));
        if (savedYear) setSelectedYear(parseInt(savedYear));
        if (savedDesignation) setDesignationFilter(savedDesignation);
    }, []);
    ...
    useEffect(() => {
        ...
        localStorage.setItem('attendance_history_designation', designationFilter);
        ...
        try {
            ...
            if (records && records.length > 0) {
                const stats = {
                    ...
                    // Count ALL employees who showed up as Present (matches employee view)
    ...
    const handleClearFilters = () => {
        ...
        localStorage.removeItem('attendance_history_designation');
                                ...
                                {attendanceRecords.map((record: any) => {
                                    ...
                                    return (
                                        <tr key={record.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                {/* Show actual status in the table */}
...
//         return {
//             total: activeEmployees.length,
//             // FIXED: Count ALL employees who showed up (Present, Late, Early Departure)
//             present: todayRecords.filter((r) => 
//                 r.status === 'Present' || r.status === 'Late' || r.status === 'Early Departure'
//             ).length,
//             absent: todayRecords.filter((r) => r.status === 'Absent').length,
...
//                                                 <td className="px-4 py-3 font-medium">{formatDisplayDate(rec.date)}</td>
//                                                 <td className="px-4 py-3">
//                                                     {/* FIXED: Show actual status, not just "Present" for all */}
//                                                     <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
//                                                         rec.status === 'Present' ? 'bg-green-100 text-green-800' :
//                                                         rec.status === 'Late' ? 'bg-yellow-100 text-yellow-800' :
//                                                         rec.status === 'Early Departure' ? 'bg-orange-100 text-orange-800' :
...
//                                 <div>
//                                     <div className="font-bold text-lg text-green-600">
//                                         {/* FIXED: Only count strictly Present, not Late or Early Departure */}
...
//                                 <div>
//                                     <div className="font-bold text-lg text-orange-600">
//                                         {/* Count Early Departure separately */}
//                                         {myRecords.filter((r) => r.status === 'Early Departure').length}
//                                     </div>
//                                     <div className="text-gray-600">Early Out</div>
//                                 </div>
...
//                 const stats = {
//                     totalDays: records.length,
//                     // FIXED: Only count strictly Present
